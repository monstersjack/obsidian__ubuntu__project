...
// node_modules/set-interval-async/dist/set-interval-async-timer.cjs
var require_set_interval_async_timer = __commonJS({
  "node_modules/set-interval-async/dist/set-interval-async-timer.cjs"(exports) {
    "use strict";
    Object.defineProperty(exports, "__esModule", { value: true });
    exports.SetIntervalAsyncTimer = void 0;
    var MIN_INTERVAL_MS = 10;
    var MAX_INTERVAL_MS = 2147483647;
    var _timeout, _promise, _stopped, _scheduleTimeout, scheduleTimeout_fn, _runHandlerAndScheduleTimeout, runHandlerAndScheduleTimeout_fn;
    var _SetIntervalAsyncTimer = class {
      constructor() {
        __privateAdd(this, _scheduleTimeout);
        __privateAdd(this, _runHandlerAndScheduleTimeout);
      ...
      static startTimer(strategy, handler, intervalMs, ...handlerArgs) {
        ...
        const timer = new _SetIntervalAsyncTimer();
        __privateMethod(_a2 = timer, _scheduleTimeout, scheduleTimeout_fn).call(_a2, strategy, handler, intervalMs, intervalMs, ...handlerArgs);
        return timer;
      }
      static async stopTimer(timer) {
        __privateSet(timer, _stopped, true);
        if (__privateGet(timer, _timeout)) {
          clearTimeout(__privateGet(timer, _timeout));
        }
        if (__privateGet(timer, _promise)) {
          await __privateGet(timer, _promise);
        }
      }
    };
    var SetIntervalAsyncTimer = _SetIntervalAsyncTimer;
    _timeout = new WeakMap();
    _promise = new WeakMap();
    _stopped = new WeakMap();
    _scheduleTimeout = new WeakSet();
    scheduleTimeout_fn = function(strategy, handler, intervalMs, delayMs, ...handlerArgs) {
      __privateSet(this, _timeout, setTimeout(async () => {
        __privateSet(this, _timeout, void 0);
        __privateSet(this, _promise, __privateMethod(this, _runHandlerAndScheduleTimeout, runHandlerAndScheduleTimeout_fn).call(this, strategy, handler, intervalMs, ...handlerArgs));
        await __privateGet(this, _promise);
        __privateSet(this, _promise, void 0);
      }, delayMs));
    };
    _runHandlerAndScheduleTimeout = new WeakSet();
    runHandlerAndScheduleTimeout_fn = async function(strategy, handler, intervalMs, ...handlerArgs) {
      ...
      try {
        ...
        if (!__privateGet(this, _stopped)) {
          ...
          const delayMs = strategy === "dynamic" ? intervalMs > executionTimeMs ? intervalMs - executionTimeMs : 0 : intervalMs;
          __privateMethod(this, _scheduleTimeout, scheduleTimeout_fn).call(this, strategy, handler, intervalMs, delayMs, ...handlerArgs);
        }
      }
    };
...
// node_modules/set-interval-async/dist/clear-interval-async.cjs
var require_clear_interval_async = __commonJS({
  "node_modules/set-interval-async/dist/clear-interval-async.cjs"(exports) {
    "use strict";
    Object.defineProperty(exports, "__esModule", { value: true });
    exports.clearIntervalAsync = void 0;
    var set_interval_async_timer_cjs_1 = require_set_interval_async_timer();
    async function clearIntervalAsync3(timer) {
    ...
    exports.clearIntervalAsync = clearIntervalAsync3;
  }
});

// node_modules/set-interval-async/dist/dynamic/set-interval-async.cjs
var require_set_interval_async = __commonJS({
  "node_modules/set-interval-async/dist/dynamic/set-interval-async.cjs"(exports) {
    "use strict";
    Object.defineProperty(exports, "__esModule", { value: true });
    exports.setIntervalAsync = exports.clearIntervalAsync = void 0;
    var clear_interval_async_cjs_1 = require_clear_interval_async();
    Object.defineProperty(exports, "clearIntervalAsync", { enumerable: true, get: function() {
      return clear_interval_async_cjs_1.clearIntervalAsync;
    } });
    var set_interval_async_timer_cjs_1 = require_set_interval_async_timer();
    function setIntervalAsync2(handler, intervalMs, ...handlerArgs) {
    ...
    exports.setIntervalAsync = setIntervalAsync2;
  }
});
...
var import_clear_interval_async = __toESM(require_clear_interval_async(), 1);
var import_set_interval_async = __toESM(require_set_interval_async(), 1);
...
var GHSyncPlugin = class extends import_obsidian.Plugin {
  ...
  async SyncNotes() {
    ...
    let statusResult = await git.status().catch((e) => {
      this.showNotice("Vault不是Git仓库，或者找不到Git二进制文件。“，”错误", 1e4);
      return;
    });
    if (!statusResult) {
      return;
    ...
    try {
      ...
      this.showNotice(conflictMsg, "警告");
      for (let c of conflictStatus.conflicted) {
  ...
  async CheckStatusOnStart() {
    try {
      ...
      if (statusUponOpening.behind > 0) {
        ...
        this.showNotice("GitHub同步：通过远程更新。“，”信息");
      }
    } catch (e) {
    }
  }
...
var GHSyncSettingTab = class extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    howto.createEl("div", { text: "如何使用此插件", cls: "howto_title" });
    ...
    linkEl.createEl("span", { text: "请参阅" });
    linkEl.createEl("a", { href: "https://github.com/kevinmkchin/Obsidian-GitHub-Sync/blob/main/README.md", text: "自述文件" });
    linkEl.createEl("span", { text: "了解更多信息和故障排除。" });
    new import_obsidian.Setting(containerEl).setName("远程URL").setDesc("“）.addText（（text）=>text.set占位符（”").setValue(this.plugin.settings.remoteURL).onChange(async (value) => {
      this.plugin.settings.remoteURL = value;
      await this.plugin.saveSettings();
    }).inputEl.addClass("my-plugin-setting-text"));
    new import_obsidian.Setting(containerEl).setName("git二进制位置").setDesc("这是可选的！仅当无法通过系统PATH找到git时才设置此项，然后在此处提供其位置。有关更多信息，请参阅README。").addText((text) => text.setPlaceholder("").setValue(this.plugin.settings.gitLocation).onChange(async (value) => {
      this.plugin.settings.gitLocation = value;
      await this.plugin.saveSettings();
    }).inputEl.addClass("my-plugin-setting-text2"));
    new import_obsidian.Setting(containerEl).setName("通知级别").setDesc("选择在Obsidian UI中显示哪些GitHub Sync通知。").addDropdown((dropdown) => dropdown.addOption("ALL", "ALL").addOption("WARNING", "WARNING").addOption("ERROR", "ERROR").setValue(this.plugin.settings.noticeLevel).onChange(async (value) => {
      this.plugin.settings.noticeLevel = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian.Setting(containerEl).setName("隐藏成功消息").setDesc("隐藏同步成功完成时显示的单个成功通知。").addToggle((toggle) => toggle.setValue(!this.plugin.settings.showSyncSuccessNotice).onChange(async (value) => {
      this.plugin.settings.showSyncSuccessNotice = !value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian.Setting(containerEl).setName("检查启动状态").setDesc("启动黑曜石时，检查您是否在遥控器后面。").addToggle((toggle) => toggle.setValue(this.plugin.settings.checkStatusOnLoad).onChange(async (value) => {
      this.plugin.settings.checkStatusOnLoad = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian.Setting(containerEl).setName("启动时自动同步").setDesc("如果有未同步的更改，则在启动Obsidian时自动与远程同步。").addToggle((toggle) => toggle.setValue(this.plugin.settings.isSyncOnLoad).onChange(async (value) => {
      this.plugin.settings.isSyncOnLoad = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian.Setting(containerEl).setName("间隔自动同步").setDesc("设置自动同步保管库的分钟间隔。如果此字段为空或不是正整数，则禁用自动同步。重新启动Obsidan以生效。").addText((text) => text.setValue(String(this.plugin.settings.syncinterval)).onChange(async (value) => {
